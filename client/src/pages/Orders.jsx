...
const Orders = () => {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'pending':
        return { icon: ClockIcon, color: 'text-yellow-600 bg-yellow-50', text: 'Pending' };
      case 'confirmed':
        return { icon: CheckCircleIcon, color: 'text-blue-600 bg-blue-50', text: 'Confirmed' };
      case 'shipped':
        return { icon: TruckIcon, color: 'text-purple-600 bg-purple-50', text: 'Shipped' };
      case 'delivered':
        return { icon: CheckCircleIcon, color: 'text-green-600 bg-green-50', text: 'Delivered' };
      case 'cancelled':
        return { icon: XCircleIcon, color: 'text-red-600 bg-red-50', text: 'Cancelled' };
      default:
        return { icon: ClockIcon, color: 'text-gray-600 bg-gray-50', text: 'Unknown' };
    }
  };
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {order.items.slice(0, 3).map((item, index) => (
                          <div key={index} className="flex items-center space-x-3">
                            <img
                              src={item.product?.images?.[0]?.url || item.image || 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAiIGhlaWdodD0iNDAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHJlY3Qgd2lkdGg9IjEwMCUiIGhlaWdodD0iMTAwJSIgZmlsbD0iI2RkZCIvPjwvc3ZnPg=='}
